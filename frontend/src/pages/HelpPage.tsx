...
function LrTrack(): JSX.Element {
  return (
    <Space direction="vertical" size={16} style={{ width: "100%" }}>
      <Card title="Откуда берутся ваши задачи" size="small">
        <ProcMeta goal="Понять, что ждёт именно вашего действия" who="LR заказчика" where="Меню: Обзор → «Текущие задачи»" />
        <Steppo steps={[
          {
            title: "Задачи — только по вашим разделам",
            body: (
              <>
                В списке задач документы тех пар «категория + раздел», на которые вы назначены в
                матрице. Назначены на «SE / SE» — видите изыскания, и не видите чужие разделы, даже
                если они в том же проекте.
              </>
            ),
            tip: "Назначили вас сегодня — документы, рассмотрение которых уже закрыто (CRS отправлен или стоит AP), в задачи не попадут: по ним круг пройден.",
          },
          {
            title: "Задача появляется, пока мяч у заказчика",
            body: (
              <>
                Статусы <Kbd>На рассмотрении заказчиком</Kbd> и <Kbd>Ответ подрядчика (принято)</Kbd>.
                Как только вы отправили CRS, мяч уходит подрядчику — задача снимается сама и
                возвращается, когда он ответит.
              </>
            ),
          },
          {
            title: "Клик по задаче открывает документ",
            body: (
              <>
                Кнопка <Kbd>Открыть</Kbd> ведёт прямо в карточку документа с PDF и замечаниями.
                Плитка <Kbd>Мои задачи</Kbd> сверху просто прокручивает к списку. Очередь ТРМ и CRS —
                отдельные экраны для пакетной работы, искать в них документ не нужно.
              </>
            ),
          },
          {
            title: "Что ещё считается задачей",
            body: (
              <>
                Непрочитанные уведомления: новое замечание от R, ответ подрядчика, приближение
                дедлайна. Открыли документ и отработали — уведомление гаснет.
              </>
            ),
            warn: "Пусто в задачах — значит по вашим разделам сейчас ничего не ждёт. Все документы проекта смотрите в меню «Документы».",
          },
        ]} />
      </Card>
      <Card title="Не хватает ревьюверов — добавьте на документ" size="small">
        <ProcMeta goal="Подключить к рассмотрению людей сверх матрицы назначений" who="LR документа (R), администратор (R или LR)" where="Карточка документа → блок «Рассмотрение ревьюверами»" />
        <Steppo steps={[
          {
            title: "Нажмите «+ Ревьювер»",
            body: (
              <>
                В карточке документа, в блоке <Kbd>Рассмотрение ревьюверами</Kbd>. Кнопка есть у LR этого
                документа и у администратора.
              </>
            ),
          },
          {
            title: "Выберите сотрудника и роль",
            body: (
              <>
                В списке — сотрудники заказчика из участников проекта, которых ещё нет в составе, включая
                наблюдателей. LR добавляет ревьювера <Kbd>R</Kbd>; второго <Kbd>LR</Kbd> назначает администратор.
              </>
            ),
            tip: "Нужного человека нет в списке — попросите администратора добавить его в участники проекта.",
          },
          {
            title: "Что произойдёт",
            body: (
              <>
                Человек подключится к текущему кругу рассмотрения и останется в составе на следующих ревизиях
                этого документа. Получит уведомление и задачу, в блоке ревьюверов у него метка
                <Kbd>добавлен</Kbd>. Матрица назначений проекта не меняется — на другие документы это не влияет.
              </>
            ),
          },
          {
            ...
            body: (
              <>
                Ссылка <Kbd>снять</Kbd> рядом с добавленным ревьювером. Оставленные им замечания сохранятся.
                Людей из матрицы так снять нельзя — это делается в матрице назначений проекта.
              </>
            ),
          },
        ]} />
      ...
      <Card title="Carry-over и закрытие документа (AP)" size="small">
        ...
        <Steppo steps={[
          ...
          {
            title: "Если подрядчик ответил «на обсуждении» (I)",
            body: (
              <>
                Подрядчик не согласен с замечанием. У вас две кнопки, и они противоположны по смыслу:
                <br />
                <Kbd>Не согласен — исправить</Kbd> — вы настаиваете: замечание остаётся обязательным, подрядчик
                должен его устранить, и ответить он сможет только «принято» (A).
                <br />
                <Kbd>Отклонить замечание</Kbd> — вы соглашаетесь с доводами подрядчика: замечание снимается,
                исправлять его не нужно.
              </>
            ),
            tip: "Наведите курсор на кнопку — подсказка повторяет это же своими словами.",
          },
...
function RTrack(): JSX.Element {
  return (
    <Space direction="vertical" size={16} style={{ width: "100%" }}>
      <Card title="Откуда берутся ваши задачи" size="small">
        <ProcMeta goal="Понять, что ждёт именно вашего действия" who="R (ревьюер) заказчика" where="Меню: Обзор → «Текущие задачи»" />
        <Steppo steps={[
          {
            title: "Только ваши разделы",
            body: (
              <>
                В задачах документы тех пар «категория + раздел», на которые вас назначили в матрице.
                Один раздел — значит и документы только этого раздела, а не весь проект.
              </>
            ),
            tip: "Не видите документ, который ждёте? Проверьте у администратора, есть ли назначение на его категорию и раздел.",
          },
          {
            title: "Вас могут подключить к отдельному документу",
            body: (
              <>
                LR документа или администратор может добавить вас ревьювером на конкретный документ, даже если
                по матрице вы на него не назначены. Придёт уведомление, документ появится в задачах, а в блоке
                ревьюверов рядом с вами будет метка <Kbd>добавлен</Kbd>.
              </>
            ),
          },
          {
            title: "Задача висит, пока ревизия на рассмотрении",
            body: (
              <>
                Пока статус <Kbd>На рассмотрении заказчиком</Kbd>, документ ждёт вашего действия:
                дать замечания или нажать <Kbd>Рассмотрено, без замечаний</Kbd>. После этого задача
                уходит, а решение видит LR.
              </>
            ),
            warn: "Кнопка остаётся на экране и после нажатия — повторные нажатия ничего не меняют и в журнал не пишутся.",
          },
          {
            title: "Клик по задаче открывает документ",
            body: (
              <>
                Кнопка <Kbd>Открыть</Kbd> ведёт в карточку документа с PDF и замечаниями — искать
                документ в списках не нужно.
              </>
            ),
          },
        ]} />
    ...
    <Space direction="vertical" size={16} style={{ width: "100%" }}>
      <Card title="Матрица назначений (кто что ревьюит)" size="small">
        ...
        <Steppo steps={[
          ...
          { title: "Добавьте строку", body: <><Kbd>+ Добавить строку матрицы</Kbd> → категория документа → раздел (для SE — «все отчёты изысканий» или конкретный вид отчёта) → сотрудник заказчика → LR или R.</>, tip: "Строка адресуется парой «категория + раздел»: дисциплина SE в категории PF и отчёты категории SE — разные документы и разные люди." },
        ]} />
      </Card>
      <Card title="Структура проекта: сделать документ вложенным или самостоятельным" size="small">
        <ProcMeta goal="Перенести документ во вложенные к другому документу или вынести на верхний уровень" who="Администратор" where="Проекты → дерево структуры проекта, или реестр документов → «Переместить»" />
        <Steppo steps={[
          {
            title: "Перетащите документ в дереве",
            body: (
              <>
                На другой документ — он станет вложенным. В промежуток между документами верхнего уровня или на
                категорию — вынесется на верхний уровень. Либо в реестре документов кнопка <Kbd>Переместить</Kbd> и
                выбор головного документа из списка.
              </>
            ),
          },
          {
            title: "Проверьте, что изменится, и подтвердите",
            body: (
              <>
                Перед переносом система покажет активную ревизию и состав ревьюверов до и после: у вложенного
                документа рассматривает состав головного. Если по документу идёт рассмотрение, текущий круг
                сразу перейдёт к новому составу.
              </>
            ),
            warn: "Шифр при переносе не меняется. Вложенность — только в один уровень: документ с вложенными сначала освободите от них.",
          },
        ]} />
      </Card>
      <Card title="Ревьюверы сверх матрицы на отдельный документ" size="small">
        <ProcMeta goal="Подключить людей к конкретному документу, не меняя матрицу" who="Администратор (R или LR), LR документа (только R)" where="Карточка документа → «Рассмотрение ревьюверами» → «+ Ревьювер»" />
        <Steppo steps={[
          {
            title: "Когда это нужно",
            body: (
              <>
                По отдельному документу состава из матрицы мало. Вместо новой строки матрицы (она затронет все
                документы пары «категория + раздел») добавьте человека только на этот документ.
              </>
            ),
            tip: "Добавленный на головной документ ревьювер рассматривает и его вложенные.",
          },
        ]} />
      </Card>
